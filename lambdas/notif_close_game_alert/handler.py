...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    for job in jobs:
        ...
        log.info(
            f"Scanning live scores for week {week} in league {league_id} "
            f"({job.source})"
        )
        ...
        # Nobody to alert, so the three Sleeper calls below are pure waste.
        if not sleeper_id_to_email:
            continue
    ...
    log.info(
        f"Close game alerts sent to {sent} managers across "
        f"{len(leagues_done)} league(s)."
    )
    return success_response(
        {"sent": sent, "week": week, "leagues": leagues_done},
        is_api=False,
    )
